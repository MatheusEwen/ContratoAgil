        ...
        DropdownMenu(
            ...
            suggestions.forEach { label ->
                DropdownMenuItem(onClick = {
                    ...
                    imovelScreenViewModel.onImovelChanged(label)
                    imovelScreenViewModel.onNomeImovelChanged(label.nomeImovel)
                    imovelScreenViewModel.onEnderecoChanged(label.endereco)
                    imovelScreenViewModel.onObsChanged(label.obs)
                    imovelScreenViewModel.onMtQuadradosChanged(label.mtQuadrados)
                    imovelScreenViewModel.onQtdBanheirosChanged(label.qtdBanheiros)
                    imovelScreenViewModel.onQtdQuartosChanged(label.qtdQuartos)
                    imovelScreenViewModel.onIdMovelChanged(label.id)
                    atualizaValor(selectedText)
                    expanded = false
